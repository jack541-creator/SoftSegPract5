...
from src.ciphercoin.autenticacion import (
...
class Boton(tk.Canvas):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.fg = fg or (
            COLORES["violeta"]
            if self.color in (COLORES["fondo_input"], COLORES["acento_soft"], COLORES["fondo_2"])
            else COLORES["texto_invertido"]
        )
        self.texto = texto
...
def aplicar_estilo_ttk():
    ...
    estilo.map(
        "Treeview",
        background=[("selected", COLORES["acento_soft"])],
        foreground=[("selected", COLORES["violeta"])],
    )
    estilo.configure(
        "Treeview.Heading",
        background=COLORES["fondo_2"],
...
class LoginFrame(tk.Frame):
    ...
    def _construir(self):
        ...
        tk.Label(
            self,
            text="La Criptomoneda para los Pequeños Negocios",
            bg=COLORES["fondo"],
            fg=COLORES["texto_suave"],
            font=("Segoe UI", 20),
        ).pack(pady=(0, 20))
        ...
        # tk.Label(demo_frame,text="Credenciales de Demo",bg=COLORES["fondo"],
        # fg=COLORES["texto_suave"],font=FUENTE_PEQUENA).pack(anchor="center", pady=(0, 6))
...
class DashboardFrame(tk.Frame):
    ...
    def _construir_acciones(self, parent):
        ...
        if self._sesion.wallet.tipo == TipoWallet.ESTADO:
            Boton(botones, "🛡 Panel Admin", self._abrir_admin, color=COLORES["estado"]).pack(
                side="left"
            )
...
class VentanaTransferencia(tk.Toplevel):
    ...
    def __init__(
        ...
        self._wallets_destino = [
            w for w in master.sistema.listar_wallets() if w.direccion != sesion.wallet.direccion
        ]
        self._construir()
        self._centrar()
    ...
    def _construir(self):
        ...
        Boton(btns, "Cancelar", self.destroy, color=COLORES["fondo_input"]).pack(
            side="left", padx=8
        )
    ...
    def _actualizar_preview(self, _event=None):
        ...
        self._var_preview.set(
            f"Comisión: {comision:.4f} ₿  |  Total a descontar: {total:.4f} ₿{rep_bonus}"
        )
    ...
    def _confirmar(self):
        ...
        try:
            tx = self._app.sistema.transferir(
                self._sesion.wallet.direccion, destino.direccion, importe
            )
...
class Appciphercoin(tk.Tk):
    ...
    def __init__(self):
        ...
        self._frame_actual: tk.Frame | None = None
        self.mostrar_login()
